  ...
  grunt.initConfig({
    ...
    svgmin: {
      dist: {
        files: [{
          expand: true,
          cwd: '<%= yeoman.dist %>',
          src: ['**/*.svg', '!**/fonts/**', '!**/img/banner-assets/**'],
          dest: '<%= yeoman.dist %>'
        }]
      }
    },
    copy: {
      dist: {
        ...
        },
        {
          expand: true,
          dot: true,
          cwd: '<%= yeoman.app %>/_bower_components/easyfont-roboto-mono',
          src: [
            // Copy css imports from _bower_components.
            // 'Open Sans fonts to dist/css folder'
            'fonts/**/*'
    ...
    buildcontrol: {
      dist: {
        options: {
          dir: 'dist',
          remote: 'git@gitlab.com:' + CONFIG.owner + '/' + CONFIG.repo + '.git',
